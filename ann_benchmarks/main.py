...
def run_worker(cpu: int, mem_limit: int, args: argparse.Namespace, queue: multiprocessing.Queue) -> None:
    ...
    while not queue.empty():
        ...
        if args.local:
            run(definition, args.dataset, args.count, args.runs, args.batch, args.skip_dataload, args.test_multiplier, args.skip_duplicate_check)
        else:
            cpu_limit = str(cpu) if not args.batch else f"0-{multiprocessing.cpu_count() - 1}"

            run_docker(definition, args.dataset, args.count, args.runs, args.timeout, args.batch, args.skip_dataload, args.test_multiplier, args.skip_duplicate_check, cpu_limit, mem_limit)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--skip-duplicate-check", action="store_true", help="If set, do not check if returned values unique")
...
def main():
    ...
    # we specify batch threads in yaml and want to distinguish runs,
    # but it is applicable to batch mode only
    if not args.batch:
        for d in definitions:
            try:
                del d.arguments[1]['threads']
            except:
                pass
